...
do
	...
	mapFile=${InFolder}/raw_fastqContigs/${mapFile::-6}-contigs.fastq
	outFile=${refFile#*blastSequences/}
	debugFile=${outFile::-5}.out
	outFile=${outdir}/${outFile::-5}-contigs.fasta
	echo "refFile"
	echo "$refFile"
	echo "sbatch -o ${debugDir}/8_remapCtigs-${debugFile} ${HERE}/.other/unpairedRemapAll.sh -r $refFile -m $mapFile -o $outdir"
	sbatch -o ${debugDir}/8_remapCtigs-${debugFile} ${HERE}/.other/unpairedRemapAll.sh -r $refFile -m $mapFile -o $outdir
	echo "mapFile"
	echo "$mapFile"
done
