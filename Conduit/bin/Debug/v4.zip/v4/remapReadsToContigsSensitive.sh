...
#$ -cwd'
#
# 	Originally Created by Reza Mazloom rmazloom@ksu.edu
#	Version 1.0.1
#	11/21/2017
#
#SBATCH --time=4:00:00
...
remap=$CWD/.other/remap_bowtieAllSensitive.sh
LOG="$CWD/logs"
...
echo
out_dir="$(echo $ref | rev | cut -f 2- -d '_' | rev)_remapReadsToContigs"
rm -rf $out_dir
mkdir $out_dir
for contig in `ls $ref/*-contigs.fasta`;
do
	echo $contig
	...
	echo "sbatch -o ${debugDir}/6_remapStep-${contig#*trimmedContigs/}.out $config -J ${sample}-remap $remap -d $(realpath $contig) -f $map/*${sample}_R1*.fastq -r $map/*${sample}_R2*.fastq -o $out_dir"
	echo
...
touch ${outdir%/raw_remap*}6_remap.done
