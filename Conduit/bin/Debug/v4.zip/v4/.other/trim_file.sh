...
# Get file count and check if its equal to the orginal
fileCount_n=$(find $OutFolder -type f -name "*.fastq" | wc -l)
if [ $fileCount_n == $fileCount_o ]
then
    touch $parentdir'/1_trim.done'
fi
