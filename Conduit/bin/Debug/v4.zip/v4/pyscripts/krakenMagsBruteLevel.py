...
try:
	...
	sys.exit(0)
print(t1)
taxLevels=['Root', 'OrganizationType', 'Domain', 'Phylum', 'Class', 'Order', 'Family', 'Genus', 'Species', 'Strain', 'Variant']

if not os.path.exists(t1+'/taxMags'):
    os.makedirs(t1+'/taxMags')

if not os.path.exists(t1+'/taxMags/all'):
    os.makedirs(t1+'/taxMags/all')


for i in range(len(taxLevels)):
	taxLevel=i
	writer = pd.ExcelWriter(t1+'/taxMags/all/'+str(i) + '_' + taxLevels[i] +'.xlsx', engine='xlsxwriter')
	for file in os.listdir(t1+'/kraken_translated'):
		...
		df=DataFrame({taxLevels[i]: sortedKeys, 'Magnitude': sortedMags, 'Percent': sortedPercent})
...
if not os.path.exists(t1+'/taxMags/viralOnly'):
	os.makedirs(t1+'/taxMags/viralOnly')

for i in range(len(taxLevels)):
	taxLevel=i
	writer = pd.ExcelWriter(t1+'/taxMags/viralOnly/' + str(i) + '-' +taxLevels[i] + '.xlsx', engine='xlsxwriter')
	for file in os.listdir(t1+'/translated_viral_only'):
		if '.translate' in file or '.viralonly' in file:
			print(file)
		else: continue
		dataOrigin=[]
		dataKeys=[]
		data={}
		with open (t1+'/translated_viral_only/'+file) as f:
			while True:
				line=f.readline()
				if not line: break
				item=line.split('\t')[1].strip()
		...
		df=DataFrame({'Domain': sortedKeys, 'Magnitude': sortedMags, 'Percent': sortedPercent})
		#df.to_excel(t1+'/taxMags/'+file.split('.')[0]+'.xlsx', sheet_name='sheet1', index=False)
		df.to_excel(writer, sheet_name=file.split('.')[0], index=False)
...
print('done')
with open(t1+"/3_taxMags.done", "w") as f:
	f.write(" ")
